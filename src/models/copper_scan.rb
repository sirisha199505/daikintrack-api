class App::Models::CopperScan < Sequel::Model
  ...
  def as_pos
    as_json(only: [
      ...
      :length_m, :gauge_system, :gauge_value, :diameter_mm, :weight_g,
      :image, :points, :notes, :actor, :status, :created_at, :updated_at
    ]).merge!(
  ...
  def as_summary
    as_json(only: [
      :id, :branch_id, :reference_type, :length_m, :gauge_system,
      :gauge_value, :diameter_mm, :weight_g, :notes, :actor, :status,
